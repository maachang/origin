...
public final class Json {
    ...
    private static final void _encode(StringBuilder buf, Object base,
        ...
        if (target instanceof Map) {
            ...
            if (target instanceof ScriptObjectMirror) {
                ScriptObjectMirror so = (ScriptObjectMirror) target;
                ...
                // Native日付の場合.
                    ...
                    buf.append("\"")
                            .append(dateToString(new java.util.Date(
                                    ((Double) so.callMember("getTime"))
                                            .longValue()))).append("\"");
                    return;
                }
            }
    ...
    /** [encodeJSON]jsonListコンバート. **/
    private static final void encodeJsonList(StringBuilder buf, Object base,
            ScriptObjectMirror list) throws IOException {
        boolean flg = false;
        buf.append("[");
        int len = list.size();
        for (int i = 0; i < len; i++) {
