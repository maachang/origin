...
// バッファを生成.
var _createBuffer = function( len ) {
...
// len 対象の長さを設定します.
// 戻り値 : 配列が返却されます.
o.createBuffer = function( len ) {
    return _createBuffer( len ) ;
}

// 圧縮処理で必要な最大バッファ長を計算.
o.calcMaxCompressLength = function( len ) {
    return _calcMaxCompressLength( len ) ;
}

// 圧縮処理(高速版).
// JSnappy.compressでは、圧縮された結果を
// 別途圧縮長の配列を生成して、コピー処理を
// 行っていますが、この処理では、それらは行いません.
// out 圧縮された内容を格納する配列を設定します.
//     通常は、JSnappy.calcMaxCompressLengthで最大バッファ長を
//     計算して、JSnappy.createBufferで生成したものを
//     セットします.
// src 圧縮対象の情報を設定します.
// off 対象のオフセット値を設定します.
// len 対象の長さを設定します.
// 戻り値 : 圧縮長が返却されます.
o.fcompress = function( out,src,off,len ) {
    off = off|0 ;
    len = len|0 ;
...
// len 対象の長さを設定します.
// 戻り値 : 圧縮された情報が返却されます.
o.compress = function( src,off,len ) {
    off = off|0 ;
    len = len|0 ;
    ...
    var out = _createBuffer( _calcMaxCompressLength( len ) ) ;
    var res = _comp( out,src,off,len ) ;
    var ret = _createBuffer( res ) ;
...
// len 対象の長さを設定します.
// 戻り値 : 圧縮された情報が返却されます.
o.compressString = function( src,off,len ) {
    var res = o.compress(src,off,len);
    var len = res.length;
    var ret = "";
    for(var i = 0; i < len; i ++) {
...
// len 対象の長さを設定します.
// 戻り値 : 解凍された情報が返却されます.
o.decompress = function( src,off,len ) {
    off = off|0 ;
    len = len|0 ;
    if( len == 0 ) {
        len = src.length ;
    ...
    var ret = _createBuffer( _decompressLength( src,off ) ) ;
...
// len 対象の長さを設定します.
// 戻り値 : 解凍された情報が返却されます.
o.decompressString = function( src,off,len ) {
    var ret = o.decompress(src,off,len);
