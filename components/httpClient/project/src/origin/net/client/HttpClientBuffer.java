 ...
 * HttpClient用バイナリバッファ.
 */
final class HttpClientBuffer extends OutputStream {
     ...
     */
    public HttpClientBuffer(boolean closeToClean) {
     ...
     *            対象の１データのバッファ長を設定します.
     */
    public HttpClientBuffer(int size) {
        this(size, false);
    }
     ...
     *            [true]を設定した場合、クローズ処理時に情報も破棄します.
     */
    public HttpClientBuffer(int size, boolean closeToClean) {
        if (size <= MIN_LENGTH) {
